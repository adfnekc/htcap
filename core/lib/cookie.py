...
class Cookie:
    ...
    def __init__(self, cookie, setter=None):
        self.name = (str(cookie['name'])
                     if 'name' in cookie and cookie['name'] else None)
        self.domain = (str(cookie['domain'])
                       if 'domain' in cookie and cookie['domain'] else None)
        self.path = (str(cookie['path'])
                     if 'path' in cookie and cookie['path'] else "/")
    ...
    def update(self, cookie):
        self.value = (str(cookie['value'])
                      if 'value' in cookie and cookie['value'] else None)
        self.expires = (cookie['expires'] if 'expires' in cookie
                        and cookie['expires'] not in (-1, '-1', '0') else None)
    ...
    def __eq__(self, other):
        return (other and self.name == other.name and self.path == other.path
                and (self.domain == None or other.domain == None
    ...
    def is_valid_for_url(self, url):
        ...
        if self.domain is None:
            ...
            if self.setter and purl.hostname != self.setter.hostname:
                return False
        else:
            if not purl.hostname: return False
            # url is valid ALSO if it is a subdomain of self.domain
            sh = [t for t in self.domain.split(".")[::-1]
                  if t]  # skip empty vals (in case of .foo.bar)
    ...
    def get_as_netscape(self):
        ...
        expiry = self.expires if self.expires else (time.time() +
                                                    (3600 * 24 * 7))
        values = (domain, flag, self.path,
                  ("TRUE" if self.secure else "FALSE"), expiry, self.name,
                  self.value)
