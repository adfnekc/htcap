...
class Request(object):
    def __init__(self,
        ...
        if type != REQTYPE_UNKNOWN:
            ...
            self.url = normalize_url(
                urljoin(parent.url, url) if parent else url)
        else:
            self.url = url
        ...
        if json_cookies:
            ...
            self.all_cookies = self.merge_cookies(
                set_cookie, parent.all_cookies) if parent else set_cookie

        self.cookies = [
            c for c in self.all_cookies if c.is_valid_for_url(self.url)
        ]
    ...
    def __eq__(self, other):
        ...
        return (self.method, self.url, self.http_auth,
