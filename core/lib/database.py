...
class Database:
    ...
    def create(self):
        ...
        try:
            ...
            cur.execute(
                "INSERT INTO crawl_info values (NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL)"
            )
    ...
    def save_request(self, request):
        ...
        values = (request.parent_db_id, request.type, request.method,
                  request.url, request.referer, request.redirects,
                  request.data,
                  json.dumps([r.get_dict() for r in request.cookies]),
                  request.http_auth if request.http_auth else "",
                  json.dumps(
                      request.extra_headers), 1 if request.out_of_scope else 0,
                  json.dumps(request.trigger) if request.trigger else "",
                  request.html if request.html else "",
                  json.dumps(request.user_output)
                  if len(request.user_output) > 0 else "")
        ...
        values_select = (request.type, request.method, request.url,
                         request.http_auth if request.http_auth else "",
                         request.data, json.dumps(request.trigger)
                         if request.trigger else "")
    ...
    def save_crawl_result(self, result, crawled):
        ...
        values = (1 if crawled else 0, json.dumps(result.errors),
                  result.request.html if result.request.html else "",
                  json.dumps(result.request.user_output)
                  if len(result.request.user_output) > 0 else "",
    ...
    def insert_vulnerability(self,
        ...
        try:
            ...
            cur.execute(qry,
                        (id_assessment, id_request, type, description, error))
            self.conn.commit()
            self.close()
    ...
    def insert_vulnerabilities(self, id_assessment, id_request,
                               vulnerabilities):
        error = ""  # @TODO
        qry = "INSERT INTO vulnerability (id_assessment, id_request, type, description, error) values (?,?,?,?,?)"
        try:
            ...
            for v in vulnerabilities:
                cur.execute(qry, (id_assessment, id_request, v['type'],
                                  v['description'], error))
            self.conn.commit()
            self.close()
