...
class Mutation:
    ...
    def set_parameter(self, param,
                      value):  #it also ulencodes the returned url !
        ...
        if ppath[0] == "get":
            ...
            self.body = self._set_json_parameter(self.request.data, param,
                                                 value)
        elif ppath[0] == "post-xml":
            self.body = self._set_xml_parameter(self.request.data, param,
                                                value)
        elif ppath[0] == "cookie":
    ...
    def send(self, req_timeout=5, ignore_errors=False):
        http = HttpGet(self.request,
                       req_timeout,
                       proxy=self._scanner.proxy,
                       useragent=self._scanner.user_agent,
                       extra_headers=self._scanner.extra_headers)
        cookies = []
        ...
        resp = http.send_request(method=self.method,
                                 url=self.url,
                                 data=self.body,
                                 cookies=cookies,
    ...
    def __str__(self):
        ...
        return "%s %s=%s  %s %s" % (self.method, self.parameter, self.payload,
                                    url, data)
...
class Mutations:
    ...
    def __next__(self):
        ...
        self._nextm = self._switch_mutation_method(
            mut) if self.switch_method else None
    ...
    def _switch_mutation_method(self, mutation):
        ...
        try:
            if self.request.method == METHOD_GET:
                ...
                mut.url, mut.body = self._get_to_post(mutation.url,
                                                      mutation.parameter)
                par[0] = "post"
    ...
    def _get_json_parameters(self, obj, path=None):
        ...
        try:
            for e in obj:
                ...
                if not obj[e] or isinstance(obj[e], int) or isinstance(
                        obj[e], float) or isinstance(obj[e], str):
                    pars.append(cp)
                else:
    ...
    def _get_xml_parameters(self, node, path=None):
        ...
        try:
            for e in node:
                cp = "%s%s" % (path + "/" if path else "post-xml/%s/" %
                               node.tag, e.tag)
                if len(e) == 0:
                    pars.append(cp)
