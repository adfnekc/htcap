...
class Scanner:
    def get_modules_file(self, path):
        ...
        return [
            os.path.basename(m).split(".")[0] for m in files
            if not m.endswith("__.py")
        ]

    def __init__(self, argv, db_file=None):
        self.scanners = self.get_modules_file(
            os.path.join(getrealdir(__file__), "scanners"))
        ...
        try:
            ...
            print(
                "Unable to read proxy, cookies and user_agent from db.. maybe db created vith an old version . . ."
            )
            pass
        ...
        try:
            mod = importlib.import_module("core.scan.scanners.%s" %
                                          self.scanner)
        except Exception as e:
            if modules_path:
                try:
        ...
        try:
            ...
            run(self.db_file, num_threads, request_types, display_progress,
