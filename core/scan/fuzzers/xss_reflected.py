...
class XssHTMLParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        if tag == "htcap-scan-tag":
            ...
            return
        elif tag in ("img", "input", "object", "link", "script", "body",
                     "frame", "frameset", "iframe",
                     "style"):  #@TODO continue list
        ...
        for key, val in attrs:
            if key[0:2] == "on" or (key == "src" and tag in (
                    "script", "img", "frame", "frameset", "iframe",
                    "object")) or (key == "href"
                                   and tag in ("a", "base", "link")) or (
                                       key == "action" and tag == "form"):
