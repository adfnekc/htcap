...
class Arachni(BaseScanner):
    ...
    def get_settings(self):
        return dict(request_types="xhr,link,form,jsonp,redirect,fetch",
                    num_threads=5,
                    process_timeout=180)
    ...
    class Scan(ScannerThread):
        ...
        def run(self):
            ...
            timeout = str(
                datetime.timedelta(
                    seconds=(self.scanner.settings['process_timeout'] - 5)))

            cmd = [
                #"--checks", "sql_injection*",
                "--checks",
                "code_injection*,file_inclusion*,path_traversal*,rfi*,xss*,xxe*",  # "xss*",
            ...
            if self.request.referer:
                cmd.extend([
                    '--http-request-header',
                    'Referer=%s' % self.request.referer
                ])

            if len(self.request.cookies) > 0:
                cmd.extend([
                    "--http-cookie-string", "; ".join([
                        "%s=%s" % (c.name, c.value)
            ...
            if self.scanner.proxy:
                ...
                cmd.extend([
                    '--http-proxy',
                    '%s:%s' %
                    (self.scanner.proxy['host'], self.scanner.proxy['port'])
                ])

            extra_headers = []
            for hn in self.request.extra_headers:
                if hn not in self.scanner.extra_headers:
                    extra_headers.append(hn + "=" +
                                         self.request.extra_headers[hn])
            ...
            if cmd_out['err']:
                print(">>> error exporting arachni to json: %s %s" %
                      (cmd_out['err'], self.request.url))
                return
            ...
            for i in issues:
                ref = i['references']['OWASP'] if i[
                    'references'] and 'OWASP' in i['references'] else "N/A"
                req = "N/A"
                req = None
