...
class Native(BaseScanner):
    def init(self, argv):
        self.fuzzers = [
            'sqli_blind', 'sqli_error', 'xss_reflected', 'cmdinjection',
            'fileinclude'
        ]
    ...
    def end(self):
        print(("\nTot vulnerabilities: %d (not vuln: %d)" %
               (len(self.vulns), self.tmp)))
        pass
    ...
    class Scan(ScannerThread):
        def run(self):
            ...
            if len(vulns) > 0:
                ...
                self.sprint("%s contains vulnerable request: %s" %
                            (parent.url if parent else "/", request))
                self.sprint("\033[31m%s %s   %s\033[0m" %
