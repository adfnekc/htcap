...
class Sqlmap(BaseScanner):
    ...
    class Scan(ScannerThread):
        def run(self):
            ...
            cmd = [
                "--batch", "-u", self.request.url, "-v", "0",
                "--disable-coloring", "--text-only", "--purge", "-o",
                "--crawl=0", "--output-dir", out_dir
            ]
            ...
            if self.scanner.proxy:
                cmd.extend(
                    ("--proxy", "%s://%s:%s" %
                     (self.scanner.proxy['proto'], self.scanner.proxy['host'],
                      self.scanner.proxy['port'])))
