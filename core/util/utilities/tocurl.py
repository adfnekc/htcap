...
class Tocurl(BaseUtil):
    @staticmethod
    def get_settings():
        return dict(descr="Export saved requests to curl arguments",
                    optargs='',
                    minargs=0,
                    use_dbfile=True)
    ...
    def main(self, args, opts, db_file):
        ...
        for req in cur.fetchall():
            cookies = [
                "%s=%s" % (c['name'], c['value'])
                for c in json.loads(req['cookies'])
            ]
            cookies_str = "Cookie: %s" % ";".join(cookies) if len(
                cookies) > 0 else ""
            method = "POST" if req['method'] == "POST" else "GET"
            referer = "Referer: %s" % req['referer'] if req['referer'] else ""
            cmd = [
                "-k", '-H', cookies_str, '-X', method, '-H', referer,
                req['url']
            ]
            if req['data']:
