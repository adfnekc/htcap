...
class Report(BaseUtil):
    ...
    @staticmethod
    def get_settings():
        return dict(descr="Generate the html report",
                    optargs='',
                    minargs=1,
                    use_dbfile=True)
    ...
    def get_json(self, cur):
        ...
        ret = dict(infos=infos, results=[])

        for row in report:
            if row['http_auth']:
                row['url'] = self.add_http_auth(row['url'], row['http_auth'])

            if self.get_request_cmp_tuple(row) in [
                    self.get_request_cmp_tuple(r) for r in ret['results']
            ]:
                continue
            d = dict(id=row['id'],
                     url=row['url'],
                     ...
                     websocket=[],
                     form=[],
                     errors=json.loads(row['crawler_errors'])
                     if row['crawler_errors'] else [],
                     vulnerabilities=self.get_assessment_vulnerabilities(
                         cur, row['id']))
            ...
            if row['has_requests']:
                for r in report:
                    ...
                    if r['req_type'] == "jsonp" and r['req_url'].find(
                            "?") == -1:
                        continue

                    trigger = json.loads(r['trigger']) if 'trigger' in r and r[
                        'trigger'] else None  # {'event':'ready','element':'[document]'}
            ...
            if row['has_requests'] or row['out_of_scope'] or len(
                    d['errors']) > 0 or len(d['vulnerabilities']) > 0:
    ...
    def main(self, args, opts, db_file):
        ...
        base_dir = os.path.dirname(
            os.path.realpath(__file__)) + os.sep + "htmlreport" + os.sep
        ...
        with open("%sreport.html" % base_dir) as html, open(
                "%sreport.js" % base_dir) as js, open("%sstyle.css" %
                                                      base_dir) as css:
