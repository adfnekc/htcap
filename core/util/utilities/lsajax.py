...
class Lsajax(BaseUtil):
    @staticmethod
    def get_settings():
        return dict(descr="List all pages and related ajax calls",
                    optargs='d',
                    minargs=0,
                    use_dbfile=True)

    def usage(self):
        return ("usage: %s <dbfile> [<sql_where_clause>]\n"
                "  Options:\n    -d    print POST data\n\n" % self.utilname)
    ...
    def main(self, args, opts, db_file):
        ...
        for res in cur.fetchall():
            ...
            trigger_str = "%s.%s() -> " % (trigger['element'],
                                           trigger['event']) if trigger else ""
            data = " data: %s" % (
                res['data']) if print_post_data and res['data'] else ""
            descr = "  %s%s %s%s" % (trigger_str, res['method'], res['url'],
                                     data)
